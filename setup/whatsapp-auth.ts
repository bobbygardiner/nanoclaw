...
async function handleQrBrowser(
  ...
  try {
    const svg = execSync(
      `node -e 'const QR=require("qrcode");let d="";process.stdin.on("data",c=>d+=c);process.stdin.on("end",()=>QR.toString(d,{type:"svg"},(e,s)=>{if(e)process.exit(1);process.stdout.write(s)}))'`,
